    ...
    >
      ...
      >
        ...
        <motion.div 
          ...
          className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 w-full max-h-[60vh] overflow-y-auto pr-2 scrollbar-thin pb-4"
        >
          {projectsData.map((project) => (
            <motion.div
              key={project.num}
              variants={cardVariants}
              className="border-2 border-[#0C0C0C] bg-[#FFFFFF] p-6 rounded-none flex flex-col justify-between min-h-[360px] h-[48vh] hover:bg-[#0C0C0C] hover:text-white transition-all duration-300 group cursor-default text-left shadow-md hover:shadow-2xl active:scale-[0.98]"
            >
              <div>
              ...
              {/* Bottom Row: Live Project Button */}
              <div className="mt-5">
                <a 
                  href={project.link} 
                  target="_blank" 
                ...
                >
                  <LiveProjectButton className="w-full flex items-center justify-center py-2 px-6 sm:py-2.5 sm:px-8 text-xs sm:text-sm" />
                </a>
              </div>
            </motion.div>
          ))}
